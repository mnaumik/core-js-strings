...
function formatTime(minutes, seconds) {
  ...
  if (minutes < 10 && seconds < 10) {
    result = `${minute.padStart(minute.length + 1, '0')}:${second.padStart(second.length + 1, '0')}`;
  } else if (minutes > 10 && seconds < 10) {
    result = `${minute}:${second.padStart(second.length + 1, '0')}`;
  } else if (minutes < 10 && seconds > 10) {
    result = `${minute.padStart(minute.length + 1, '0')}:${second}`;
  } else {
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < result.length; i += 1) {
    count.push(result[i].length);
  }

  for (let i = 0; i < result.length; i += 1) {
    if (result[i].length === Math.max(...count)) {
      final.push(result[i]);
    }
  }
  return final[0];
